...
class Calculator extends Component{
 ...
 lastInt = (x)=>{
  this.setState({last:x})
 }
  handleClick = (event)=>{
    this.lastInt(event.target.value)
...
handleEquals= ()=>{
  this.setState((prevState)=>{
    ...
    return {history:prevState.history+'='+result, operator:'=',input:result,result:0}
  }
  if(prevState.last=='+'){
    if(prevState.result> 0 && prevState.operator!='='){
      result = Number(prevState.result)+Number(prevState.input)
     return {history:result, operator:'=',input:result,result:0}
   }
  }
  }) 
}
